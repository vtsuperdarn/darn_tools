...
register_projection(SDFoVCarto)


def radar_overlay_plot(date, rad, fname, coords="geo", figsize=(5,5), dpi=120):
    import matplotlib.pyplot as plt
    fig = plt.figure(figsize=figsize, dpi=dpi)
    ax = fig.add_subplot(projection="sdfovcarto",\
            coords=coords, rad=rad, plot_date=date)
    ax.coastlines()
    ax.overlay_radar()
    ax.overlay_fov()
    ax.grid_on(draw_labels=[False, False, True, True])
    ax.enum()
    if fname is not None: fig.savefig(fname, bbox_inches="tight")
    return fig, ax